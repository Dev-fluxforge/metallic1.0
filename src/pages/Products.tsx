  ...
  return (
    <div className="pt-32 pb-20">
      <div className="container mx-auto px-4">
        ...
        {/* Filters Bar */}
        <div className="bg-card p-6 rounded-[2rem] shadow-sm mb-12 flex flex-col lg:flex-row items-center gap-8 border border-border-subtle">
          <div className="relative flex-1 w-full">
            ...
            <input 
              ...
              className="w-full bg-brand-light border-none rounded-full px-14 py-4 focus:ring-2 focus:ring-brand-gold transition-all text-brand-dark"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <button 
              ...
              className={cn(
                ...
                activeCategory === 'All' ? "bg-brand-dark text-brand-light" : "bg-brand-light text-gray-500 hover:bg-brand-gold/10 hover:text-brand-gold"
              )}
            >
              All
            </button>
            {CATEGORIES.map(cat => (
              <button 
                ...
                className={cn(
                  "px-6 py-3 rounded-full text-sm font-bold transition-all",
                  activeCategory === cat.name ? "bg-brand-dark text-brand-light" : "bg-brand-light text-gray-500 hover:bg-brand-gold/10 hover:text-brand-gold"
                )}
              >
                {cat.name}
              </button>
          ...
          <div className="flex items-center gap-4 w-full lg:w-auto">
            ...
            <select 
              ...
              className="flex-1 lg:w-48 bg-brand-light border-none rounded-full px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-brand-gold transition-all appearance-none text-brand-dark"
            >
        ...
        {sortedProducts.length === 0 && (
          <div className="py-32 text-center">
            <div className="w-24 h-24 bg-card rounded-full flex items-center justify-center mx-auto mb-8 shadow-sm border border-border-subtle">
              <Search className="w-10 h-10 text-gray-200" />
            </div>
            <h3 className="text-2xl font-bold mb-2 text-brand-dark">No products found</h3>
            ...
            <button 
              ...
              className="bg-brand-dark text-brand-light px-8 py-4 rounded-full font-bold"
            >
              Clear All Filters
            </button>
          </div>
